...
这个仓库就是按下文所说的方式建成的,所以它自己的历史就是最快的解释。

一个人想要一份理论,写出来、发布出去。他没有动笔,没有做计划,也没有管理过程。他只做了三件事。他说了要什么、做到什么样算完——这是 **`intent`**。他登录了 GitHub、装好工具、批准了发布——这是 **`grant`**。他读了交回来的东西,说「行」,或者指出哪里不对——这是 **`verdict`**。其余一切由 AI 完成:调研、写作、翻译、建仓。而每当 AI 需要只有人能给的东西——一项权限、一个后果长远的决定——它交回来的是一份简短的、解释充分的请求,而不是一个原始的问题。

结果出偏时,头号嫌疑永远是目标的表述,而不是 AI 的努力程度。人抓到 AI 把一个旧决定(许可证)悄悄带进已经变化的局面时,这次抓获成了一条常设规则。人说出「有些地方我看不懂」时,这个 verdict——按本文自己的规则——判的是文档的罪,不是读者的。本节因此而存在。
